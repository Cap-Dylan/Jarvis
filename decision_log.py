...
# Override via DECISIONS_FILE env var (used in docker-compose to keep
# the log on a mounted volume separate from the application code).
DEFAULT_LOG_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "decisions.jsonl"
)
LOG_FILE = os.environ.get("DECISIONS_FILE", DEFAULT_LOG_PATH)
MAX_ENTRIES = 500
...
def log_decision(event, reasoning, action, details=None):
    ...
    # Make sure the parent directory exists (matters for first run in
    # a container where ./data is freshly bind-mounted and empty).
